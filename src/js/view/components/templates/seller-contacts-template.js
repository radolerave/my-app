  ...
  logic: async (args) => {
    ...
    for(let i = 0; i < contactsKeys.length; i++) {
      ...
      if(contactTypeData.length > 0) {
        theSellerContacts += `<div contact-type="${contactType}">`
          ...
          switch(contactType) {
            case "phones":
              if(typeof element.phone != "undefined" && element.phone.length > 0) {
                ...
                switch(element.phoneType) {
                  //WhatsApp
                  case 3: actions.push(/*html*/`
                      <ion-item class="ion-no-padding open-with-whatsapp" button="true" phone-number="${propertyValue.replace(/\s/g, "")}">
                        <ion-icon slot="end" name="logo-whatsapp"></ion-icon>
                        <ion-label style="white-space: pre-wrap;">Ouvrir avec l'application WhatsApp</ion-label>
                      </ion-item>
                    `)
                    break
                  //Skype
                  case 4: actions.push(/*html*/`
                      <ion-item class="ion-no-padding open-with-skype" button="true" phone-number="${propertyValue.replace(/\s/g, "")}">
                        <ion-icon slot="end" name="logo-skype"></ion-icon>
                        <ion-label style="white-space: pre-wrap;">Ouvrir avec l'application Skype</ion-label>
                      </ion-item>
                    `)
                  break

                  default: actions.push(/*html*/`
